...
st.markdown(
    "<div style='opacity:0.45;font-size:0.85rem;margin-bottom:1.5rem;'>"
...
st.markdown(
    "<div style='padding:1rem;background:rgba(128,128,128,0.06);border-radius:8px;"
    "border:1px solid rgba(128,128,128,0.15);margin-bottom:1rem;'>"
...
with col1:
    ...
    for line in [
        ...
        st.markdown(f"<div style='font-size:0.85rem;opacity:0.45;padding:0.2rem 0;'>📊 {line}</div>",
                    unsafe_allow_html=True)
...
with col2:
    ...
    st.markdown(
        ...
        "<span style='opacity:0.55;'>Worsening 7 days. Likely onboarding bug.</span><br>"
...
st.markdown(
    "<div style='font-size:0.85rem;opacity:0.45;margin-bottom:0.8rem;'>"
...
for num, name, color, subtitle, desc in layers:
    st.markdown(
        ...
        f"<div>"
        f"<div style='font-weight:600;'>{name} <span style='opacity:0.55;"
        f"font-weight:400;font-size:0.85rem;'>— {subtitle}</span></div>"
        f"<div style='font-size:0.85rem;opacity:0.6;margin-top:0.2rem;'>{desc}</div>"
        f"</div></div>",
        unsafe_allow_html=True,
    )
...
for title, desc in sections:
    st.markdown(
        f"<div style='padding:0.4rem 0;border-bottom:1px solid rgba(128,128,128,0.1);'>"
        f"<span style='font-weight:600;'>{title}</span>"
        f"<span style='opacity:0.55;font-size:0.85rem;'> — {desc}</span>"
        f"</div>",
        unsafe_allow_html=True,
    )
...
for key, arch in ARCHETYPES.items():
    st.markdown(
        ...
        f"margin-bottom:0.5rem;border-left:3px solid rgba(21,101,192,0.7);'>"
        f"<div style='font-weight:600;'>{arch['emoji']} {arch['name']}</div>"
        f"<div style='font-size:0.82rem;opacity:0.65;margin-top:0.2rem;'>{arch['description']}</div>"
        f"<div style='font-size:0.78rem;opacity:0.45;margin-top:0.2rem;'>"
...
for code, name, desc in scenarios:
    st.markdown(
        ...
        f"<div style='font-family:monospace;font-size:0.8rem;opacity:0.45;min-width:1rem;'>{code}</div>"
        f"<div><strong style='font-size:0.85rem;'>{name}</strong>"
        f"<span style='opacity:0.55;font-size:0.82rem;'> — {desc}</span></div>"
        f"</div>",
        unsafe_allow_html=True,
    )
